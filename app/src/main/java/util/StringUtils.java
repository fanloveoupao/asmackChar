...
public abstract class StringUtils {
     ...
     * lowercase at the same time, use {@link #capitalize(String)}.
     * </p>
     * <p/>
     * <p>
    ...
    /**
     * Joins all the strings in the list in a single one separated by the
     * separator sequence.
     *
     ...
     * The ellipsis will be taken into account when counting the amount of
     * characters.
     *
        ...
        if (isNotBlank(text)) {
            ...
            if (text.length() > maxCharacters) {
                if (truncateWords) {
                    ...
                    // The words are truncated and the ellipsis is added when is
                    // possible
                    if (maxCharacters <= StringUtils.ELLIPSIS.length()) {
                        truncatedTextBuilder.append(text.substring(0,
                                maxCharacters));
                    } else {
                        truncatedTextBuilder.append(text.substring(0,
                                maxCharacters - StringUtils.ELLIPSIS.length()));
                    ...
                    while (it.hasNext() && !exit) {
                        String word = it.next();
                        int increment = usedChars == 0 ? word.length() : word
                                .length() + 1;
                        if ((usedChars + increment) <= maxCharacters) {
                            truncatedTextBuilder.append(usedChars == 0 ? word
                                    : SPACE + word);
                            usedChars += increment;
                        } else {
                            exit = true;
    ...
    /**
     * Truncate the text adding "...". The ellipsis will be taken into account
    ...
    public static Set<String> extractPlaceHolders(String string) {
        Matcher matcher = Pattern.compile(StringUtils.PLACEHOLDER_PATTERN)
                .matcher(string);
    ...
    /**
     * Transform the received value removing all the not alphanumeric or spaces
     * characters
     *
    ...
    public static String toAlphanumeric(String value) {
        return Pattern.compile(StringUtils.ALPHANUMERIC_PATTERN).matcher(value)
                .replaceAll("");
    }
    ...
    /**
     * Returns the first token of the string if that string can be split by the
     * token, else return the unmodified input string
     *
    ...
    /**
     * Returns the first token of the string if that string can be split by a
     * ",", else return the unmodified input string
     *
     * @param string The string to split
     * @return The resultant string
     */
    ...
    /**
     * This method word wrap a text to two lines if the text has multiple words
     * and its length is greater than the specified minLength. To do the word
     * wrap, the white space closest to the middle of the string is replaced by
     * a "\n" character
     *
    ...
    public static String wordWrapToTwoLines(String text, int minLength) {
        ...
        if ((wordWrapText != null) && (wordWrapText.length() > minLength)) {
            ...
            int leftSpaceIndex = wordWrapText.substring(0, middle).lastIndexOf(
                    SPACE);
        ...
        while (matcher.find()) {
            ...
            if (replacement == null) {
                throw new IllegalArgumentException(
                        "Template contains unmapped key: " + key);
            }
    ...
    /**
     * To convert a string into ASCII
     *
     * @param s
     * @return
     */
    public static String stringToASCII(String s) {
        ...
        String string = (int) chars[0] + "";
        try {
            for (int i = 1; i < chars.length; i++) {// 输出结果
                string = string + " " + (int) chars[i];
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public static String trimEnd(String str, String regularExpression) {
        if (str != null && str.length() > 0 && str.length() > regularExpression.length()) {
            if (str.substring(str.length() - regularExpression.length(), str.length()).equals(regularExpression)) {
                return str.substring(0, str.length() - regularExpression.length());
            }
        }
        return str;
    }
