...
public class CropWrapper {
    ...
    public static void camera(File tmpDir, android.support.v4.app.Fragment fragment) {
        File tmpFile = getTempFileWithTmpDir(tmpDir);
        if (tmpFile == null) {
            sCameraSaveUri = null;
            Crop.camera(fragment);
    ...
    public static Uri onActivityResult(Context context, File tmpDir, int requestCode, int resultCode, Intent data) {
...
//                        data.getData());
                ...
                // 读取图片偏转角度
                int angle = BitmapUtils.getExifRotation(getTempFileWithTmpDir(tmpDir),
                        sourceBitmap);
                // 切小
